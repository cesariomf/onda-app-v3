...
function acervoCarregar() { try { return JSON.parse(localStorage.getItem(ACERVO_KEY) || '[]'); } catch { return []; } }
function acervoSalvar(entrada) {
  try {
    const lista = acervoCarregar();
    const dup = lista.find(e => e.artista === entrada.artista && e.musica === entrada.musica && (Date.now() - new Date(e.data).getTime()) < 86400000);
    if (dup) return dup;
    const nova = { id: Date.now().toString(36) + Math.random().toString(36).slice(2,6), data: new Date().toISOString(), ...entrada };
    localStorage.setItem(ACERVO_KEY, JSON.stringify([nova, ...lista].slice(0, 200)));
    return nova;
  } catch { return null; }
}
function acervoRemover(id) { try { localStorage.setItem(ACERVO_KEY, JSON.stringify(acervoCarregar().filter(e => e.id !== id))); } catch {} }
    ...
    <div style={{minHeight:"100vh", background:C.bg, fontFamily:C.corpo, color:C.creme, overflowX:"hidden"}}>
      ...
      <div style={{maxWidth:640, margin:"0 auto", padding:"0 24px 80px"}}>
        ...
        <div style={{marginTop:44,textAlign:"center",animation:"fadein 1s ease 0.8s both"}}>
          ...
          {onVerAcervo && <button onClick={onVerAcervo} style={{background:'none',border:'none',color:'#C9A84C',fontSize:'13px',cursor:'pointer',marginTop:'12px',opacity:0.7,display:'block',width:'100%',textAlign:'center'}}>ver meu acervo</button>}
        </div>
      </div>
    </div>
  );
...
function TelaAcervo({ onVoltar }) {
  const [lista, setLista] = React.useState(() => acervoCarregar());
  const [sel, setSel] = React.useState(null);
  const [artigo, setArtigo] = React.useState(false);
  const remover = (id) => { acervoRemover(id); setLista(acervoCarregar()); if (sel?.id===id) setSel(null); };
  const fmtData = (iso) => { try { return new Date(iso).toLocaleDateString('pt-BR',{day:'2-digit',month:'short',year:'numeric'}); } catch { return iso; } };
  const bg = '#0D0D12', fg = '#F4F0E0', gold = '#C9A84C', muted = '#888';
  if (sel && artigo) return (
    <div style={{minHeight:'100vh',background:bg,color:fg,padding:'24px 20px',fontFamily:'Crimson Pro,Georgia,serif'}}>
      <button onClick={()=>setArtigo(false)} style={{background:'none',border:'none',color:gold,cursor:'pointer',fontSize:'14px',marginBottom:'24px'}}>← voltar ao acervo</button>
      <div style={{maxWidth:'680px',margin:'0 auto'}}>
        <p style={{fontSize:'12px',color:muted,textTransform:'uppercase',letterSpacing:'0.1em',marginBottom:'8px'}}>{sel.artista}</p>
        <h2 style={{fontFamily:'Playfair Display,serif',fontSize:'22px',marginBottom:'4px'}}>{sel.musica}</h2>
        <p style={{fontSize:'13px',color:gold,marginBottom:'24px'}}>{sel.ilha} · {fmtData(sel.data)}</p>
        <div style={{fontSize:'17px',lineHeight:'1.8',whiteSpace:'pre-wrap'}}>{sel.artigo}</div>
      </div>
    </div>
  );
  if (sel) {
    const info = Object.values(ILHAS_SISTEMA).find(i=>i.nome===sel.ilha)||{};
    return (
      <div style={{minHeight:'100vh',background:bg,color:fg,padding:'24px 20px',fontFamily:'Crimson Pro,Georgia,serif'}}>
        <button onClick={()=>setSel(null)} style={{background:'none',border:'none',color:gold,cursor:'pointer',fontSize:'14px',marginBottom:'24px'}}>← voltar</button>
        <div style={{maxWidth:'400px',margin:'0 auto',textAlign:'center'}}>
          <div style={{width:'80px',height:'80px',borderRadius:'50%',background:info.cor||gold,margin:'0 auto 20px',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'32px'}}>{info.emoji||'🎵'}</div>
          <p style={{fontSize:'12px',color:muted,textTransform:'uppercase',letterSpacing:'0.15em',marginBottom:'8px'}}>{sel.artista}</p>
          <h2 style={{fontFamily:'Playfair Display,serif',fontSize:'24px',marginBottom:'8px'}}>{sel.musica}</h2>
          <p style={{fontSize:'15px',color:info.cor||gold,marginBottom:'8px'}}>{sel.ilha}</p>
          <p style={{fontSize:'14px',color:'#AAA',lineHeight:'1.6',marginBottom:'28px'}}>{sel.fraseSintese}</p>
          <p style={{fontSize:'12px',color:'#666',marginBottom:'28px'}}>{fmtData(sel.data)}</p>
          {sel.artigo && <button onClick={()=>setArtigo(true)} style={{background:'none',border:'1px solid '+gold,color:gold,padding:'10px 24px',borderRadius:'20px',cursor:'pointer',fontSize:'14px',marginBottom:'16px',width:'100%'}}>ler artigo do Maestro</button>}
          <button onClick={()=>setSel(null)} style={{background:'none',border:'none',color:'#555',cursor:'pointer',fontSize:'13px'}}>voltar</button>
        </div>
      </div>
    );
  }
  return (
    <div style={{minHeight:'100vh',background:bg,color:fg,padding:'24px 20px',fontFamily:'Crimson Pro,Georgia,serif'}}>
      <div style={{maxWidth:'600px',margin:'0 auto'}}>
        <button onClick={onVoltar} style={{background:'none',border:'none',color:gold,cursor:'pointer',fontSize:'14px',marginBottom:'28px'}}>← voltar</button>
        <h1 style={{fontFamily:'Playfair Display,serif',fontSize:'26px',marginBottom:'6px'}}>Seu acervo</h1>
        <p style={{fontSize:'14px',color:muted,marginBottom:'32px'}}>{lista.length} {lista.length===1?'consulta salva':'consultas salvas'}</p>
        {lista.length===0 && <p style={{color:'#555',textAlign:'center',marginTop:'60px'}}>Ainda não há nada aqui.<br/>Faça sua primeira consulta ao Maestro.</p>}
        {lista.map(item => {
          const info = Object.values(ILHAS_SISTEMA).find(i=>i.nome===item.ilha)||{};
          return (
            <div key={item.id} style={{borderBottom:'1px solid #1E1E2A',padding:'16px 0',display:'flex',alignItems:'center',gap:'16px'}}>
              <div onClick={()=>setSel(item)} style={{width:'40px',height:'40px',borderRadius:'50%',background:info.cor||'#333',flexShrink:0,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'18px'}}>{info.emoji||'🎵'}</div>
              <div style={{flex:1,cursor:'pointer'}} onClick={()=>setSel(item)}>
                <p style={{fontSize:'13px',color:muted,marginBottom:'2px'}}>{item.artista}</p>
                <p style={{fontSize:'16px',marginBottom:'2px'}}>{item.musica}</p>
                <p style={{fontSize:'12px',color:info.cor||gold}}>{item.ilha} · {fmtData(item.data)}</p>
              </div>
              <div style={{display:'flex',gap:'8px',flexShrink:0}}>
                {item.artigo && <button onClick={()=>{setSel(item);setArtigo(true);}} style={{background:'none',border:'1px solid #333',color:'#AAA',padding:'6px 10px',borderRadius:'8px',cursor:'pointer',fontSize:'12px'}}>artigo</button>}
                <button onClick={()=>remover(item.id)} style={{background:'none',border:'none',color:'#444',cursor:'pointer',fontSize:'16px',padding:'4px'}}>×</button>
              </div>
            </div>
          );
        })}
...
export default function Onda() {
  ...
  const onResultado=(mus,hist,resultado)=>{
    ...
    setNovaIlha(ilha);
        acervoSalvar({ artista: musicaPedida?.artista||'', musica: musicaPedida?.titulo||'', ilha, fraseSintese: infoIlha?.desc||'', artigo: resultado, entradaTipo: 'artista' });
  ...
  if(tela==="landing") return <OndaLanding onVerAcervo={() => setVerAcervo(true)} onEntrar={()=>setTela("inicio")}/>;
  ...
  // Tela Sessão a Dois
  if(verAcervo) return <TelaAcervo onVoltar={() => setVerAcervo(false)} />;
  if(verDuo) return (
